...
CVGridCell {
    ...
    *new {
    ...
    drawFunc {
        |v|
        Pen.fillColor_(Color.hsv(0.555, 1, 0.5 + 0.05, 0.3));
        Pen.fillRect(Rect(0, 0, v.bounds.width, v.bounds.height));
        Pen.fillColor_(Color.hsv(0.555, 1, 0.5 + 0.2));
        Pen.fillRect(Rect(0, 0, v.bounds.width * cv.input, v.bounds.height));
    }
    
    update {
        if (cv.notNil && view.notNil) {
            title.string = name.asString.toUpper();
            //		title.sizeHint;
